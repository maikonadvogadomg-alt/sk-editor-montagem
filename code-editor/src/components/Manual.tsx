...
<script>window.addEventListener("load",function(){setTimeout(function(){window.print()},300)});</script>
  ...
  }, [pwaCheckUrl, pwaCheckLoading]);

  const leftVerificadorRef = useRef(false);

  useEffect(() => {
    if (installTab !== "verificador") {
      leftVerificadorRef.current = true;
    } else if (leftVerificadorRef.current) {
      leftVerificadorRef.current = false;
      if (pwaCheckResult) runPwaCheck();
    }
  }, [installTab, pwaCheckResult, runPwaCheck]);

  useEffect(() => {
    if (!pwaCheckResult) return;
    const handler = () => {
      if (document.visibilityState === "visible" && installTab === "verificador" && active === "instalar") {
        runPwaCheck();
      }
    };
    document.addEventListener("visibilitychange", handler);
    return () => document.removeEventListener("visibilitychange", handler);
  }, [pwaCheckResult, installTab, active, runPwaCheck]);
          ...
          "verificador": (
            <div>
              ...
              {pwaCheckResult && (() => {
                ...
                const sortByPriority = (arr: typeof items) =>
                  arr.filter(i => i.status !== "ok").slice().sort((a, b) => {
                ...
                return (
                  <div>
                    <div className={`border rounded-lg p-4 mb-4 flex items-center gap-4 ${scoreBg}`}>
                      ...
                      <div className="flex flex-col gap-1.5 shrink-0">
                        <button
                          onClick={runPwaCheck}
                          disabled={pwaCheckLoading}
                          className="bg-[#1a3d14] hover:bg-[#2d4a1e] disabled:opacity-50 text-[#7ec87a] font-semibold text-xs px-3 py-2 rounded transition-colors whitespace-nowrap border border-[#3d6e2a]"
                          title="Roda a verificação novamente com a mesma URL"
                        >
                          {pwaCheckLoading ? "⏳..." : "🔄 Verificar novamente"}
                        </button>
                        <button
                          onClick={downloadPwaReport}
                          className="bg-[#2d4a1e] hover:bg-[#3d5e2a] text-[#7ec87a] font-semibold text-xs px-3 py-2 rounded transition-colors whitespace-nowrap"
                    ...
                    {score < 80 && (() => {
                      ...
                      const failed = sortByPriority(items);
                      ...
                      return (
                        <Alert color="yellow">
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-base">⭐</span>
                            <strong>
                              Quase lá! Falt{failed.length === 1 ? "a 1 item" : `am ${failed.length} itens`} — você está a um passo de estar pronto para instalar.
                            </strong>
                          </div>
                          <p className="text-xs opacity-75 mb-2">Próximo item a corrigir:</p>
                          <div className="flex gap-3 items-start">
                            <span className="bg-[#4a3800] text-[#d4aa40] font-bold rounded-full w-5 h-5 flex items-center justify-center text-xs shrink-0 mt-0.5">1</span>
                            <div className="flex-1 min-w-0">
                              <div className="font-semibold">{next.label}</div>
                              {next.fix && (
                                <div className="text-xs opacity-75 mt-0.5 leading-relaxed">{next.fix}</div>
                              )}
                            </div>
                          </div>
                        </Alert>
