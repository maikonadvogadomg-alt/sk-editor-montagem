...
import {
  ...
  Scale, Gavel, FileText, BookOpen, MessageCircle,
} from "lucide-react";
...
type LegalMode = "livre" | "advogado" | "promotor" | "magistrado" | "resumo";

const LEGAL_MODES: { id: LegalMode; label: string; color: string; icon: React.ReactNode; prompt: string }[] = [
  {
    id: "livre",
    label: "Livre",
  ...
  },
  {
    id: "promotor",
    label: "Promotor",
    color: "text-red-400 border-red-600/50 bg-red-900/20",
    icon: <Gavel size={12} />,
...
export default function CampoLivre({ onBack }: CampoLivreProps) {
  const [legalMode, setLegalMode] = useState<LegalMode>(() => (localStorage.getItem("cl_legal_mode") as LegalMode) || "livre");
  ...
  useEffect(() => {
    localStorage.setItem("cl_legal_mode", legalMode);
  }, [legalMode]);
  ...
  const sendMessage = useCallback(async (textOverride?: string) => {
    ...
    // Sistema de prompt jurídico baseado no modo ativo
    const activeLegalMode = LEGAL_MODES.find(m => m.id === legalMode);
    const legalSystemPrompt = activeLegalMode?.prompt || "";

    // Sistema de prompt de voz: pede respostas naturais (sem markdown)
    const voiceSystemMsg = ttsOn
      ? "Você está em modo de conversa por voz. Responda de forma natural e direta, SEM usar markdown, asteriscos, sustenidos, hashtags, tabelas, listas numeradas, código ou caracteres especiais. Fale como numa conversa normal. Seja objetivo e amigável."
      : "";

    const combinedSystem = [legalSystemPrompt, voiceSystemMsg].filter(Boolean).join("\n\n");

    const systemMessages: { role: "system"; content: string }[] = combinedSystem
          ...
          try {
            ...
            if (delta) {
              ...
              if (ttsOn && conversationModeRef.current) {
                ...
                if (sentMatch) {
                  const toSay = cleanForSpeech(sentMatch[1]);
  ...
  }, [prompt, apiKey, apiUrl, apiModel, history, ttsOn, ttsConfig, isProcessing, restartMicAfterSpeech, legalMode]);
  ...
  const activeModeInfo = LEGAL_MODES.find(m => m.id === legalMode)!;
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-[#141c0d] text-gray-200">
      <header className="h-11 flex items-center gap-2 px-3 bg-[#1c2714] border-b border-gray-700/50 shrink-0">
        ...
        <span className={`text-[10px] px-2 py-0.5 rounded-full border shrink-0 flex items-center gap-1 ${activeModeInfo.color}`}>
          {activeModeInfo.icon}
        ...
        {ttsOn && (
          <button
            ...
            className={`p-1.5 rounded-lg ${showVoicePanel ? "text-green-300 bg-green-900/30" : "text-gray-600 hover:bg-white/5 hover:text-green-400"}`}
      ...
      {/* ── Barra de Modos Jurídicos ── */}
      <div className="flex items-center gap-1 px-2 py-1.5 bg-[#161f0f] border-b border-gray-700/30 shrink-0 overflow-x-auto scrollbar-none">
        {LEGAL_MODES.map(mode => (
          <button
            key={mode.id}
            onClick={() => setLegalMode(mode.id)}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-semibold whitespace-nowrap transition-all border shrink-0 ${
              legalMode === mode.id
        ...
        <div className="border-b border-green-800/30 bg-[#1a2410] p-3 space-y-3 shrink-0">
          ...
          <div>
            ...
            {voiceList.length === 0 ? (
              ...
              <div className="max-h-36 overflow-y-auto space-y-0.5 rounded-lg border border-gray-700/40 bg-[#141c0d] p-1">
          ...
          <button
            ...
            className="w-full py-1.5 text-[11px] bg-green-900/30 border border-green-700/40 text-green-400 rounded-lg hover:bg-green-900/50 transition-colors"
          >
            ▶ Testar voz agora
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {allMessages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center gap-3 text-center py-12 px-4">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${activeModeInfo.color}`}>
              {activeModeInfo.icon && <span className="scale-150">{activeModeInfo.icon}</span>}
            </div>
            <div>
              <p className="text-gray-300 text-sm font-semibold">
                {legalMode === "livre" && "Campo Livre — Sem restrições"}
                {legalMode === "advogado" && "Modo Advogado"}
                {legalMode === "promotor" && "Modo Promotor"}
                {legalMode === "magistrado" && "Modo Magistrado"}
                {legalMode === "resumo" && "Modo Resumo Jurídico"}
              </p>
              <p className="text-gray-600 text-[11px] mt-1">
                {legalMode === "livre" && "Converse sobre qualquer assunto."}
                {legalMode === "advogado" && "IA responde como advogado de defesa."}
                {legalMode === "promotor" && "IA responde como promotor de justiça."}
                {legalMode === "magistrado" && "IA responde como juiz de direito."}
                {legalMode === "resumo" && "Cole um texto jurídico para resumir."}
              </p>
            </div>
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              {legalMode === "livre" && ["Olá, como você está?", "Me ajude a pensar em algo", "Conta uma curiosidade"].map(s => (
              ...
              {legalMode === "advogado" && ["Quais são meus direitos nessa situação?", "Como elaborar uma defesa?", "Me explique o habeas corpus"].map(s => (
                <button key={s} onClick={() => setPrompt(s)} className="text-[11px] px-3 py-1.5 rounded-full border border-blue-700/50 text-blue-500 hover:border-blue-500/70 hover:text-blue-300 transition-colors">{s}</button>
              ))}
              {legalMode === "promotor" && ["Analise esta conduta criminosa", "Quais crimes se aplicam aqui?", "Bases para a denúncia"].map(s => (
                <button key={s} onClick={() => setPrompt(s)} className="text-[11px] px-3 py-1.5 rounded-full border border-red-700/50 text-red-500 hover:border-red-500/70 hover:text-red-300 transition-colors">{s}</button>
              ))}
              {legalMode === "magistrado" && ["Analise este caso de forma imparcial", "Fundamente uma decisão sobre isso", "Quais princípios aplicar?"].map(s => (
                <button key={s} onClick={() => setPrompt(s)} className="text-[11px] px-3 py-1.5 rounded-full border border-yellow-700/50 text-yellow-600 hover:border-yellow-500/70 hover:text-yellow-300 transition-colors">{s}</button>
              ))}
              {legalMode === "resumo" && ["Resuma esta petição:", "Extraia os pontos principais:", "Simplifique este contrato:"].map(s => (
                <button key={s} onClick={() => setPrompt(s)} className="text-[11px] px-3 py-1.5 rounded-full border border-purple-700/50 text-purple-500 hover:border-purple-500/70 hover:text-purple-300 transition-colors">{s}</button>
              ))}
            </div>
          </div>
        )}
