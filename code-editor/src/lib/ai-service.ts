...
// ─── Chave Gemini para modo standalone (GitHub Pages / APK) ──────────────────
export function getGeminiFallbackKey(): string {
  try { return localStorage.getItem("sk-gemini-fallback-key") || ""; } catch { return ""; }
}
export function setGeminiFallbackKey(key: string) {
  try { localStorage.setItem("sk-gemini-fallback-key", key.trim()); } catch {}
}

// ─── IA embutida (cortesia via servidor, com fallback Gemini standalone) ──────
export async function sendBuiltinAI(
  ...
  // Tenta o backend do servidor primeiro
  try {
    ...
    // Se o servidor retornou erro (ex: 404 em standalone), cai no fallback
  } catch {
    // Servidor indisponível (APK/GitHub Pages) — usa fallback Gemini
  }

  // Fallback: usa chave Gemini salva localmente (funciona no APK e GitHub Pages)
  const fallbackKey = getGeminiFallbackKey();
  if (fallbackKey) {
    const geminiSlot: AIKeySlot = {
      id: 99,
      name: "Jasmim Standalone",
      provider: "google",
      apiKey: fallbackKey,
  ...
  throw new Error(
    "STANDALONE_NO_KEY: Para usar a Jasmim no APK ou fora do Replit, configure sua chave Gemini gratuita (obtenha em aistudio.google.com/apikey)."
  );
}

// ─── Verifica se o backend está disponível (útil para detectar modo standalone) ─
export async function checkBackendAvailable(): Promise<boolean> {
  try {
    const r = await fetch("/api/ai/ping", { method: "GET" });
    return r.ok;
  } catch {
    return false;
  }
